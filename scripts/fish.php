...
$browser->request('GET', 'https://animalcrossing.fandom.com/wiki/Fish_(New_Horizons)');
...
$columns = [ 'name', 'image', 'price', 'location', 'shadow' ];
...
foreach ($south->filter('tr') as $tr) {
    ...
    $cells = $td->each($td_text);
    $name = $cells[0];
    $row = reset(array_filter($rows, fn($row) => strcasecmp($row->name, $name) === 0));
