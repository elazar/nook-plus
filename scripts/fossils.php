<?php

require __DIR__ . '/../vendor/autoload.php';

use Symfony\Component\BrowserKit\HttpBrowser;
use Symfony\Component\DomCrawler\Crawler;
use Symfony\Component\HttpClient\HttpClient;

$browser = new HttpBrowser(HttpClient::create());
$browser->request('GET', 'https://animalcrossing.fandom.com/wiki/Fossils_(New_Horizons)');
$crawler = $browser->getCrawler();
$tables = $crawler->filter('table.sortable');

$parse_row = function ($td) {
    $columns = [ 'name', 'image', 'price' ];
    $td_text = fn($td) => $td->text();
    $url_data = fn($url) => strpos($url, 'data:') !== 0;

    $cells = $td->each($td_text);

    $urls = $td->eq(1)->filter('img')->eq(0)->extract(['src', 'data-src'])[0];
    $cells[1] = reset(array_filter($urls, $url_data));

    $cells[2] = (int) preg_replace('/[^0-9]/', '', $cells[2]);

    $row = (object) array_combine($columns, $cells);

    $link = $td->eq(0)->filter('a')->eq(0)->extract(['href'])[0];
    $row->link = 'https://animalcrossing.fandom.com' . $link;

    return $row;
};

$rows = [];

$single = $tables->eq(0);
foreach ($single->filter('tr') as $tr) {
    $tr = new Crawler($tr);
    $td = $tr->filter('td');
    if (!count($td)) {
        continue;
    }

    $rows[] = $parse_row($td);
}

$multi = $tables->eq(1);
foreach ($multi->filter('tr') as $tr) {
    $tr = new Crawler($tr);

    $a = $tr->filter('th > a');
    if (count($a)) {
        $category = $a->text();
        $link = 'https://animalcrossing.fandom.com' . $a->attr('href');
        continue;
    }

    $td = $tr->filter('td');
    if (!count($td)) {
        continue;
    }

    $row = $parse_row($td);
    $row->category = $category;
    $row->link = $link;

    $rows[] = $row;
}

echo json_encode($rows, \JSON_PRETTY_PRINT);
